...
export const getStudentProgress = asyncHandler(async (req, res, next) => {
  ...
  const progressData = evaluations.map(eval => ({
    date: eval.evaluatedAt,
    subject: eval.exam.subject,
    score: eval.percentage,
    grade: eval.grade,
    examTitle: eval.exam.title
  }));

  // Subject-wise progress
  const subjectProgress = {};
  evaluations.forEach(eval => {
    const subject = eval.exam.subject;
  ...
  const averageScore = evaluations.reduce((sum, eval) => sum + eval.percentage, 0) / totalEvaluations;
